...
var criticalCssOptions = {
  ...
  userAgent: args[8],
  renderWaitTime: parseInt(args[9], 10),
  auth: args[10] != 'none' ? args[10] : null,
  debugMode: args[11] === 'true',
}
...
function prepareNewPage () {
  ...
  if (criticalCssOptions.auth) {
    var auth = criticalCssOptions.auth.split(':');
...
function pruneNonCriticalCss (astRules, forceInclude, renderWaitTime, doneStatus) {
  ...
  var renderWaitTime = renderWaitTime // ms
